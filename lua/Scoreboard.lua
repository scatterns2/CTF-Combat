...
function Scoreboard_SetPlayerData(clientIndex, entityId, playerName, teamNumber, score, kills, deaths, resources, isCommander, isRookie, status, isSpectator, assists, steamId, playerSkill, currentTech, level)
...
local function UpdatePlayerRecords()
    ...
    for _, playerInfo in ientitylist(Shared.GetEntitiesWithClassname("PlayerInfoEntity")) do
		...
		elseif playerInfo.status == kPlayerStatus.Assault then
			status = Locale.ResolveString("STATUS_ASSAULT")
		elseif playerInfo.status == kPlayerStatus.Medic then
			status = Locale.ResolveString("STATUS_MEDIC")
		elseif playerInfo.status == kPlayerStatus.Engineer then
			status = Locale.ResolveString("STATUS_ENGINEER")
		elseif playerInfo.status == kPlayerStatus.Scout then
        ...
        Scoreboard_SetPlayerData(playerInfo.clientId, playerInfo.playerId, playerInfo.playerName, playerInfo.teamNumber, playerInfo.score,
                                 ...
                                 status, playerInfo.isSpectator, playerInfo.assists, playerInfo.steamId, playerInfo.playerSkill, playerInfo.currentTech, playerInfo.level)
