...
Script.Load("lua/bots/GameModeActions.lua")
...
local function GetAttackUrgency(bot, mem)
    ...
    local immediateThreats = {
		...
		[kMinimapBlipType.Assault] = true,
		[kMinimapBlipType.Engineer] = true,
		[kMinimapBlipType.Medic] = true,
		[kMinimapBlipType.Scout] = true,
    ...
    local urgencies = {
		...
		[kMinimapBlipType.Medic] =              numOthers >= 4 and 0.6 or 1,
		[kMinimapBlipType.Assault] =            numOthers >= 4 and 0.7 or 1,
		[kMinimapBlipType.Engineer] =           numOthers >= 4 and 0.6 or 1,
		[kMinimapBlipType.Scout] =              numOthers >= 4 and 0.8 or 1,
		...
		[kMinimapBlipType.Flag] =               numOthers >= 3 and 1 or 2,
...
kSkulkBrainActions =
{
    ...
    CreateGameModeAction( false, function( pos, targetPos, bot, brain, move )
                bot:GetMotion():SetDesiredMoveTarget(targetPos)
                bot:GetMotion():SetDesiredViewTarget(nil)
            end ),
    ...
    function(bot, brain)
        ...
        local name = "buyUpgrades"
        local alien = bot:GetPlayer()
        local sdb = brain:GetSenses()
        local upgradeId = GetRandomAlienUpgradeToBuy(bot)
        local weight = 0.0
