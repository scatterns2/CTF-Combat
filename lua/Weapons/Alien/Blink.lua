...
function Blink:GetCanShadowStep()
    return true
end
...
function Blink:GetIsBlinking()
    ...
    if player then
        return player.GetIsBlinking ~= nil and player:GetIsBlinking()
    end
...
function Blink:SetEthereal(player, state)
    ...
    if player.ethereal ~= state then
        ...
        if state then
            ...
            local celerityLevel = GetCelerityLevel(self)
...
function Blink:OnUpdateAnimationInput(modelMixin)
    ...
    local player = self:GetParent()
    if self:GetIsBlinking() and (not self.GetHasVoidShieldAnimationDelay or not self:GetHasVoidShieldAnimationDelay()) then
