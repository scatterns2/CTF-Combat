...
local function ResetBCount(self)
	self.bcount = 0
end
...
function Rifle:OnPrimaryAttack(player)
    ...
    if not self:GetIsReloading() then
		...
		if Shared.GetTime() >= self.timeAttackFired + Rifle.kCooldown then
			ResetBCount(self)
		end
...
function Rifle:GetIsPrimaryAttackAllowed(player)
	...
	if self.shooting and not attackAllowed then
		self.shooting = false
	end
...
function Rifle:GetIsAllowedToShoot(player)
    ...
    return player and self.clip > 0 and timeSinceAttackKeyReleased < self:GetMaxAllowedDelaySinceAttackRequest() and self.bcount < Rifle.kBulletsPerBurst
end
...
function Rifle:UpdateAttackEffects(deltaTime)
	...
	local shootingLastFrame = self.timeAttackFired + deltaTime >= Shared.GetTime()
	if not self.shooting and shootingLastFrame then
