...
function Pistol:GetBulletDamageForRange(distance)

    local damage = kPistolMaxDamage
    if distance > kPistolMaxDamageRange then
    
        local distanceFactor = (distance - kPistolMaxDamageRange) / (kPistolMinDamageRange - kPistolMaxDamageRange)
        local dmgScalar = 1 - Clamp(distanceFactor, 0, 1) 
        damage = kPistolMinDamage + dmgScalar * (kPistolMaxDamage - kPistolMinDamage)
...
function Pistol:GetBulletDamage(target, endPoint)
    ...
    if target ~= nil then
		local player = self:GetParent()
        local distance = (endPoint - player:GetEyePos()):GetLength()
		//Print("Distance to target:" .. distance)
        return self:GetBulletDamageForRange(distance)
        
    else
        //Print("Pistol:GetBulletDamage(target): target is nil, returning max damage.")
    end
    
    return kPistolMaxDamage
