...
Script.Load("lua/Weapons/Marine/ReloadSpeedMixin.lua")
...
function ClipWeapon:ProcessMoveOnWeapon(player, input)
    ...
    if player and HasMixin(player, "SprintMixin") and player:GetIsSprinting() then
...
// Sets the ammo based on the percentage. 100 = max ammo.
// Allocate to the clip first, then to the reserve.
...
local function FireBullets(self, player)
    ...
    for bullet = 1, numberBullets do
        ...
        local damage = self:GetBulletDamage(trace.entity, trace.endPoint)
...
function ClipWeapon:OnReload(player)
    ...
    if self:CanReload() then
		...
		if HasMixin(self, "VariableReloadSpeed") then
			self:TriggerEffects("reload_speed" .. self:GetReloadSpeedLevel())
		else
