...
function Babbler:OnInitialized()
    ...
    if Server then
        ...
        self:AddTimedCallback(Babbler.MoveRandom, kUpdateMoveInterval + 0.4 * math.random())
...
if Server then
    ...
    function Babbler:MoveRandom()
        ...
        if self.moveType == kBabblerMoveType.None and self:GetIsOnGround() then
            ...
            --elseif owner and (self:GetOrigin() - owner:GetOrigin()):GetLength() > 12 then                
            ...
            elseif self:GetIsOnGround() then
                ...
                if self.timeLastJump + 0.5 < Shared.GetTime() then
                    ...
                    if targetPosition then
                        ...
                        if done or (self:GetOrigin() - targetPosition):GetLengthXZ() <= 1 then 
                            if self.physicsBody then
                                self.physicsBody:SetCoords(self:GetCoords())
                            end
                            self:SetMoveType(kBabblerMoveType.None)
        ...
        if self.physicsBody and not self.doesGroundMove then
            ...
            if self.lastVelocity ~= nil then
                ...
                if delta:GetLengthSquaredXZ() > 0.0001 then
                    ...
                    local endPoint = self.lastOrigin + self.lastVelocity * (deltaTime + Babbler.kAttackRange)
                    

                    local trace = Shared.TraceCapsule(self.lastOrigin, endPoint, Babbler.kAttackRange, 0, CollisionRep.Damage, PhysicsMask.Bullets, EntityFilterOneAndIsa(self, "Babbler")) 
