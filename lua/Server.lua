...
Server.combatSpawnList = table.array(128)
...
Server.flagSpawnPoints = table.array(20)

Server.cystSpawnPoints = table.array(32)
Server.cragSpawnPoints = table.array(20)
Server.shiftSpawnPoints = table.array(20)
...
function GetCreateEntityOnStart(mapName, groupName, values)
    ...
    return mapName ~= "prop_static"
	   ...
	   and mapName ~= CombatSpawn.kMapName
	   ...
	   and mapName ~= Crag.kMapName
	   and mapName ~= Shift.kMapName
	   ...
	   and mapName ~= Sentry.kMapName
   	   and mapName ~= Flag.kMapName
	   and mapName ~= CapturePoint.kMapName
...
function GetLoadSpecial(mapName, groupName, values)
	...
	elseif mapName == CombatSpawn.kMapName then
    
        local entity = CombatSpawn()
        entity:OnCreate()
        LoadEntityFromValues(entity, values)
        table.insert(Server.combatSpawnList, entity)
        success = true
	...
	elseif mapName == Shift.kMapName then
    
        table.insert(Server.shiftSpawnPoints, values.origin)
        success = true
	...
	elseif mapName == Flag.kMapName then
	
		table.insert(Server.flagSpawnPoints, values.origin)
		success = true	
	...
	elseif mapName == CapturePoint.kMapName then
	
		local picker = GetGamerulesPicker()
		// Only show CapturePoints in Capture game mode.
		if picker:GetGameType() == kCombatGameType.CombatCapture then
...
function OnMapPreLoad()
	...
	for t = 1, #tags do
		...
		if string.find(tags[t], "ServerStatus") then
			Server.RemoveTag(tags[t])
		end
			
	end

	Server.AddTag("ServerStatus" .. 2)
...
local function OnCheckConnectionAllowed(userId)
    ...
    local numPlayers = GetGamerules():GetNumHumansConnected()
