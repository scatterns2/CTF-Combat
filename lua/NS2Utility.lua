...
function GetTexCoordsForTechId(techId)
    ...
    if not gTechIdPosition then
		...
		gTechIdPosition[kTechId.Cannon] = kDeathMessageIcon.Rifle
		...
		// Combat stuff
		gTechIdPosition[kTechId.LayLaserMines] = kDeathMessageIcon.Mine
		gTechIdPosition[kTechId.MarineStructureAbility] = kDeathMessageIcon.MarineStructureAbility
		gTechIdPosition[kTechId.Devour] = kDeathMessageIcon.Devour
		gTechIdPosition[kTechId.BabblerBombAbility] = kDeathMessageIcon.BabblerBomb
		gTechIdPosition[kTechId.SporeMineAbility] = kDeathMessageIcon.SporeMine
...
function FormatMapName(mapName)
	-- Only look for these at the beginning of the map name... ^ achieves this.
...
function GetInstalledMapList()
	...
	local mapNamesForDuplicateCheck = {}
    local mapNames = { }
    local mapFiles = { }
    ...
    for _, mapFile in pairs(matchingFiles) do
        ...
        local mapname = FormatMapName(filename):gsub("^%l", string.upper)
		
        if mapNamesForDuplicateCheck[mapname] == nil then
        
			mapNamesForDuplicateCheck[mapname] = true
