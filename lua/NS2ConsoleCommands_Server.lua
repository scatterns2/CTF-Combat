...
local function OnCommandFilm(client)
    ...
    if player:GetTeamNumber() == kTeamReadyRoom or player:GetTeamNumber() == kSpectatorIndex then
...
local function OnCommandDevouredPlayer(client)
...
local function DevourEscapeNow(player)
	local mapName = player:GetRespawnMapName(player:GetTeamNumber())
	local oldHealth = player:GetHealth()
	local oldArmor = player:GetArmor()
	newPlayer = player:Replace(mapName, player:GetTeamNumber(), false,  player:GetOrigin())
	...
	if oldWeapon1 then
		oldWeapon1:SetAmmoPercent(50)
	end
	local oldWeapon2 = newPlayer:GetWeaponInHUDSlot(2)
	if oldWeapon2 then
		oldWeapon2:SetAmmoPercent(50)
	end
end

local function OnCommandDevourEscape(client)
        
	local player = client:GetControllingPlayer()
	if Shared.GetCheatsEnabled() then
		player:SetIsOnosDying(true)
		player:AddTimedCallback(DevourEscapeNow, 1.5)
	end
...
local function OnCommandSetXpMultiplier(client, value)

	if Shared.GetCheatsEnabled() then
		kGlobalXpMultiplier = tonumber(value)
		Shared.Message("Global xp multiplier set to " .. kGlobalXpMultiplier)
	end
...
Event.Hook("Console_xpmult", OnCommandSetXpMultiplier)
...
Event.Hook("Console_devour", OnCommandDevouredPlayer)
Event.Hook("Console_devoured_player", OnCommandDevouredPlayer)
Event.Hook("Console_devouredplayer", OnCommandDevouredPlayer)
Event.Hook("Console_devourescape", OnCommandDevourEscape)
