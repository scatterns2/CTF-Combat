...
//
//    Modified by: James Gu (twiliteblue) on 11 Feb 2015
//
...
Lerk.kAirBrakeSpeedDecrease = 8
Lerk.kAirBrakeSpeedMax = 8
...
local kMaxSpeed = 11
kAirStrafeMaxSpeed = 6
local kAirStrafeAccel = 24
...
function Lerk:GetDesiredAngles()
    ...
    if not self:GetIsOnGround() and self.gliding then
        desiredAngles.pitch = math.sin( -self:GetVelocity().y / self:GetVelocityLength() ) // = self.viewPitch
    end 
...
function Lerk:GetAirFriction()
    return 0.1 - GetCelerityLevel(self) * 0.02
end
...
local function UpdateFlap(self, input, velocity)
    ...
    if flapPressed ~= self.flapPressed then
        ...
        if flapPressed and self:GetEnergy() > kLerkFlapEnergyCost then
        
            // take off
            if self:GetIsOnGround() or input.move:GetLength() == 0 then
                velocity.y = Clamp(velocity.y * 0.6, -kMaxSpeed, kMaxSpeed) + 5
                ...
                move.x = move.x * 0.75
                // flap only at 75% speed sideways
                ...
                elseif input.move.z == 1 and input.move.x == 0 then
                    flapForce = 3 + flapForce + GetCelerityLevel(self) * 0.3               
...
local function UpdateGlide(self, input, velocity, deltaTime)
    ...
    local holdingGlide = bit.band(input.commands, bit.bor( Move.Jump, Move.MovementModifier ) ) ~= 0 and self.glideAllowed
    if input.move.z == 1 and holdingGlide then
    
        local useMove = Vector(input.move)
        useMove.x = useMove.x * 0.7
        ...
        // when speed falls below 6, set horizontal speed to 1, and vertical speed to zero, but allow dive to regain speed
        if useSpeed <= kAirStrafeMaxSpeed then
			
			//lerk cannot gain altitude while gliding slowly, but can glide downward to regain speed
			local newY = math.min(wishDir.y, math.max(0, currentDir.y - deltaTime * 0.4))
			
			useSpeed = math.min(kAirStrafeMaxSpeed, useSpeed + deltaTime * 3)
			
			wishDir = GetNormalizedVectorXZ(wishDir)
			...
			local turnRate = ConditionalValue(wishDir.y > 0.707, 10, 16)		
			wishDir = GetNormalizedVector(currentDir + wishDir * deltaTime * turnRate)
...
local function UpdateAirBrake(self, input, velocity, deltaTime)
    ...
    if input.move.z ~= 0 and holdingShift then
        ...
        if velocity:GetLengthSquared() > self.kAirBrakeSpeedMax * self.kAirBrakeSpeedMax then
			local newScale = math.max(velocity:GetLength() - (self.kAirBrakeSpeedDecrease * deltaTime), self.kAirBrakeSpeedMax)            
            velocity:Normalize()
...
local function UpdateAirStrafe(self, input, velocity, deltaTime)
    ...
    if not self:GetIsOnGround() and not self.gliding then
        ...
        velocity:Add(wishDir * kAirStrafeAccel * deltaTime)
...
function Lerk:PreUpdateMove(input, runningPrediction)
	...
	local wallGripDesired = self:GetVelocity():GetLengthXZ() <= self.kWallGripMaxSpeed or input.move.z == 0
