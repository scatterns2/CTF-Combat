...
Script.Load("lua/DeathStreakMixin.lua")
...
AddMixinNetworkVars(DeathStreakMixin, networkVars)
...
function Skulk:OnCreate()
	...
	InitMixin(self, DeathStreakMixin)
...
function Skulk:ModifyJump(input, velocity, jumpVelocity)
    ...
    if self:GetCanWallJump() then
        ...
        local celerityMod = GetCelerityLevel(self) * 0.4
...
function Skulk:GetAirFriction()
    return 0.06 - GetCelerityLevel(self) * 0.009
end 
...
function Skulk:GetAnimateDeathCamera()
	// COMBAT MODE: TRYING THIS OUT.
	// Enables third person camera on death.
	return false
end
